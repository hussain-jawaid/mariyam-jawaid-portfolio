...
const contactFormSchema = z.object({
  ...
  inquiryType: z.enum(['collaboration', 'opportunity', 'general'], {
  ...
  const onSubmit = async (data: ContactFormValues) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/YOUR_FORM_ID', {
        ...
        body: JSON.stringify({
          ...data,
          _subject: `New ${data.inquiryType} inquiry from ${data.name}`,
        }),
      });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormField control={form.control} name="inquiryType" render={({ field }) => (
          <FormItem>
            ...
            <Select onValueChange={field.onChange} defaultValue={field.value}>
              <FormControl>
                <SelectTrigger className="font-light"><SelectValue placeholder="Select inquiry type" /></SelectTrigger>
              </FormControl>
              <SelectContent className="bg-popover z-50">
                <SelectItem value="collaboration" className="font-light">Collaboration</SelectItem>
                <SelectItem value="opportunity" className="font-light">Career Opportunity</SelectItem>
                <SelectItem value="general" className="font-light">General Inquiry</SelectItem>
              </SelectContent>
            </Select>
            <FormMessage className="text-xs font-light" />
          </FormItem>
        ...
        <FormField control={form.control} name="message" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Textarea placeholder="Tell me about your inquiry..." className="min-h-32 font-light resize-none" {...field} /></FormControl>
            <FormMessage className="text-xs font-light" />
          </FormItem>
        )} />
